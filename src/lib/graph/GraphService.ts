...
import cytoscape, { Core, CollectionReturnValue, NodeSingular, ElementDefinition } from 'cytoscape';
import { 
  GraphJSON, 
  NodeType, 
  EdgeType, 
...
import { initPlugins, setupAutomove } from './plugins';
import { 
  edgeExists, 
  createNoteElementDef, 
  createClusterElementDef, 
  createEdgeElementDef,
  getCurrentTimestamp,
  collectionToElementDefinitions
} from './utils';
import { Note, Cluster } from '@/lib/store';
import { slug } from '@/lib/utils';
import { generateNodeId, ClusterId } from '@/lib/utils/ids';

/**
 * Implementation of the Graph Service
 */
export class GraphService implements IGraphService {
  ...
  private changeListeners: Array<ChangeListener> = [];
  ...
  constructor() {
    ...
    // Setup automove for nodes
    setupAutomove(this.cy);
   ...
   */
  private queueNotify(defs: ElementDefinition[], changeType?: string): void {
    this.pendingChanges.push(...defs);

    const MAX_BUFFER = 200;
    if (this.pendingChanges.length >= MAX_BUFFER) {
      this.flushNotify(changeType);
      return;
    }
   ...
   */
  private flushNotify(changeType?: string): void {
    if (this.pendingChanges.length > 0) {
      const changes = [...this.pendingChanges];
      this.pendingChanges = [];
      this.changeListeners.forEach(l => l(changes, changeType));
    }
  }
  ...
  public exportGraph(opts: { includeStyle?: boolean } = {}): GraphJSON {
    ...
    const g: GraphJSON = {
      meta: { app: 'BlockNote Graph', version: 2, exportedAt: getCurrentTimestamp() },
      data: this.cy.data(),
  ...
  public importGraph(g: GraphJSON): void {
    ...
    try {
      ...
      if (g.style && Array.isArray(g.style)) {
        this.cy.style().fromJson(g.style).update();
      }
    ...
    const elementDefs = this.cy.elements().jsons();
    this.queueNotify(elementDefs, "import");
  }

  /**
   * Export a single element
   */
  public exportElement(ele: ElementDefinition | CollectionReturnValue): CyElementJSON {
    if ('json' in ele && typeof ele.json === 'function') {
      return ele.json();
    }
    return ele as CyElementJSON;
  }
  ...
  public importElement(json: CyElementJSON): void {
    ...
    this.queueNotify([json], "importElement");
  }
  ...
  public clearGraph(): void {
    const removed = this.cy.elements().jsons();
    this.cy.elements().remove();
    this.titleIndex.clear();
    this.clusterExists.clear();
    if (removed.length > 0) {
      this.queueNotify(removed, "clear");
    }
    this.initializeGraph();
  }
   ...
   * Get nodes by type
   */
  public getNodesByType(type: NodeType): cytoscape.NodeCollection {
  ...
  public addNote({ id, title, content = [], createdAt, updatedAt, path }: {
    ...
    const elementDef = createNoteElementDef(
      nodeId, 
      title, 
      content, 
      createdAt, 
    ...
    const addedElements = this.ur.do('add', [elementDef]);
    this.titleIndex.set(slug(title), nodeId);
    ...
    if (clusterId) {
      if (this.clusterExists.has(clusterId)) {
        ...
        this.queueNotify([newNode.json()], "addNote");
      }
    } else {
  ...
  public moveNodeToCluster(nodeId: string, clusterId?: string): boolean {
    ...
    this.ur.action('moveNodeToCluster', {
      ...
      return { 
        ...
        removedEdges: args.removedEdges, 
    ...
    changes.push(node.json());

    const actionResult = this.ur.lastAction?.();
    if (actionResult?.result) {
      if (actionResult.result.removedEdges) changes.push(...actionResult.result.removedEdges);
      if (actionResult.result.addedEdges) changes.push(...actionResult.result.addedEdges);
    }

    if (changes.length > 0) {
      this.queueNotify(changes, "moveNodeToCluster");
    }
  ...
  /**
   * Import notes and clusters from the store
   */
  public importFromStore(notes: Note[], clusters: Cluster[]): void {
    ...
    try {
      ...
      clusters.forEach(cluster => {
        if (!cluster.id) {
          ...
          cluster.id = generateNodeId() as ClusterId;
        }
        this.clusterExists.add(cluster.id);
        elements.push(createClusterElementDef(
          cluster.id, 
          cluster.title,
          cluster.createdAt,
          cluster.updatedAt
      ...
      notes.forEach(note => {
        ...
        elements.push(createNoteElementDef(
          note.id,
          note.title,
          note.content,
          note.createdAt,
      ...
      notes.forEach(note => {
        ...
        if (note.parentId && this.cy.getElementById(note.parentId).nonempty()) {
          edgeElements.push(createEdgeElementDef(
            note.parentId,
            note.id,
            EdgeType.CONTAINS
          ));
    ...
    this.queueNotify(this.cy.elements().jsons(), "importFromStore");
  }

  /**
   * Export the graph to store format
   */
  ...
  public updateNote(id: string, updates: Partial<Note>): boolean {
    ...
    this.queueNotify([node.json()], "updateNote");
    return true;
  }
  ...
  public deleteNote(id: string): boolean {
    ...
    const removedElements = removedCollection.jsons();
    if (removedElements.length > 0) {
      this.queueNotify(removedElements, "deleteNote");
    } else {
      this.queueNotify([removedJson], "deleteNote");
    }
  ...
  public addCluster({ id, title, createdAt, updatedAt }: Partial<Cluster> = {}): NodeSingular {
    const clusterId = id && String(id).length >= 15 ? id : generateNodeId() as ClusterId;
    ...
    const elementDef = createClusterElementDef(
      clusterId, 
    ...
    this.queueNotify([elementDef], "addCluster");
  ...
  public updateCluster(id: string, updates: Partial<Cluster>): boolean {
    ...
    this.queueNotify([node.json()], "updateCluster");
    return true;
  }
  ...
  public deleteCluster(id: string): boolean {
    ...
    const actionResult = this.ur.lastAction?.();
    const changedElements = actionResult?.result?.removedElements || [removedJson];
    if (changedElements.length > 0) {
      this.queueNotify(changedElements, "deleteCluster");
    }
   ...
   */
  public moveNode(nodeId: string, newParentId?: string | null): boolean | void {
    const node = this.cy.getElementById(nodeId);
    if (node.empty()) {
    ...
    this.queueNotify([node.json()], "moveNode");
    return true;
  }
   ...
   * Search for nodes
   */
  public searchNodes(query: string, types: NodeType[]): cytoscape.NodeCollection {
    if (!query || query.trim() === '') {
      return this.cy.collection().nodes();
    }
   ...
   * Get related nodes
   */
  public getRelatedNodes(nodeId: string): cytoscape.NodeCollection {
    const node = this.cy.getElementById(nodeId);
    if (node.empty()) return this.cy.collection().nodes();
  ...
  public tagNote(noteId: string, tagName: string): boolean {
    ...
    this.ur.action('tagNote', { noteId, tagId, tagName },
      (args) => {
        ...
        if (!edgeExists(this.cy, args.noteId, args.tagId, EdgeType.HAS_TAG)) {
          const edgeDef = createEdgeElementDef(
            args.noteId, 
            args.tagId, 
            EdgeType.HAS_TAG
      ...
      (undoArgs) => {
        if (undoArgs.addedElements && undoArgs.addedElements.length > 0) {
          const idsToRemove = undoArgs.addedElements.map((def: any) => def.data?.id);
    ...
    const actionResult = this.ur.lastAction?.();
    const changedElements = actionResult?.result?.addedElements || [];
    if (changedElements.length > 0) {
      this.queueNotify(changedElements, "tagNote");
    }
   ...
   */
  public getConnections(nodeId: string): ConnectionsResult {
    const node = this.cy.getElementById(nodeId);
    ...
    const getConnectedTargets = (edgeType: EdgeType): any[] => {
      ...
      const outgoingEdges = node.connectedEdges(`[label = "${edgeType}"][source = "${nodeId}"]`);
